...
import { assertEquals, equal, getRandomString, describe, beforeAll, afterAll, it } from '../test.deps.ts';
...
describe("Find Query Unit Testing", () => {
    ...
    let FindModel: Model<IFindModel> = (undefined as unknown) as Model<IFindModel>;
    ...
    it("Should Find All the documents", async () => {
        ...
        for (const doc of docs) {
            equal({ name: rawdocs[index].name, status: rawdocs[index].status }, { name: doc.name, status: doc.status });
            index++
        }
